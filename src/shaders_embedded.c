...
"    vec2 tile_coordinates_as_float = camera_position + (uv - 0.5) * vec2(aspect_ratio * camera_zoom, camera_zoom);\n"
"\n"
"    ivec2 tile_coordinates = ivec2(floor(tile_coordinates_as_float));\n"
"    vec2 tile_uv = fract(tile_coordinates_as_float);\n"
"\n"
"    if (tile_coordinates.x >= 0 && tile_coordinates.y >= 0 && tile_coordinates.x < CHUNK_SIZE && tile_coordinates.y < CHUNK_SIZE) {\n"
"        FragColor = texture(tile_textures, vec3(tile_uv, get_tile_id(tile_coordinates.y * CHUNK_SIZE + tile_coordinates.x)));\n"
"    } else {\n"
...
"}\n"
"\n"
"int euclideanMod(int x, int n) {\n"
"    return x - n * (x / n);\n"
"}\n"
"\n"
"float euclideanMod(float x, float n) {\n"
